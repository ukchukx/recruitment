  ...
  module.controller('hrApplicantsController', ['$scope','$http','infogathering','user_session', function($scope, $http, datagrab, user_session) {
   ...
   $http.get("http://"+datagrab.dirlocation+"hr_api/get_all_"+type+'?category='+category+'&&state='+state+'&&age='+age+'&&search_name='+search_name)
   ...
   $scope.get_search_result_applicants = function(type,category,state,age,edu_qualification,edu_start_year,edu_end_year,edu_classification){
    //////////FETCH ALL APPLICANTS
   $http.get("http://"+datagrab.dirlocation+"hr_api/get_all_"+type+'?category='+category+'&&state='+state+'&&age='+age+'&&edu_qualification='+edu_qualification+'&&edu_start_year='+edu_start_year+'&&edu_end_year='+edu_end_year+'&&edu_classification='+edu_classification)
   .then(function(response) {
    //alert(response.data);
   $scope.applicants = response.data; 
   //alert(JSON.stringify($scope.applicants));
  ...
  window.open('http://'+datagrab.dirlocation+'hr/print_result?type='+type+'&&category='+category+'&&state='+state+'&&age='+age+'&&edu_qualification='+edu_qualification+'&&edu_start_year='+edu_start_year+'&&edu_end_year='+edu_end_year+'&&edu_classification='+edu_classification,'Print Result', 'width=700,height=900');
   }

   $scope.get_all_states = function(category){
    //////////FETCH ALL RELATED TABLES  
   $http.get("http://"+datagrab.dirlocation+"hr_api/get_all_states")
   .then(function(response) {
   ...
   $http.get("http://"+datagrab.dirlocation+"hr_api/get_all_"+category+'?category='+category+'&&state='+state)
   .then(function(response) {
   $scope.applicants = response.data; 
   },function errorCallback(response) {
   return response.status;
   ...
   $scope.compare = function(prop, val){
    ...
    //////////FETCH RESULTS
   $http.get("http://"+datagrab.dirlocation+"recruit_api/get_positions?id=")
   .then(function(response) {
